...
const PatientProfile = async (props: ParamsProps) => {
    ...
    const patientFullData = await api.patient.getPatientFullDataById(id);
            ...
            <div className='w-full xl:w-3/4'>
                <div className='flex w-full flex-col gap-4 lg:flex-row'>
                    <Card className='flex w-full flex-col items-center rounded-xl border-none bg-white p-4 lg:w-[30%]'>
                        <ProfileImage
                            bgColor={patientFullData?.colorCode ?? '0000'}
                            className='h-20 w-20 md:flex'
                            name={`${patientFullData?.firstName} ${patientFullData?.lastName}`}
                            textClassName='text-3xl'
                            url={patientFullData?.image ?? ''}
                        />
                        <h1 className='mt-2 font-semibold text-2xl'>{`${patientFullData?.firstName} ${patientFullData?.lastName}`}</h1>
                        <span className='text-gray-500 text-sm'>{patientFullData?.email}</span>

                        <div className='mt-4 flex w-full items-center justify-center gap-2'>
                            <div className='w-1/2 space-y-1 text-center'>
                                <p className='font-medium text-xl'>{patientFullData?.totalAppointments}</p>
                    ...
                    <Card className='w-full space-y-6 rounded-xl border-none bg-white p-6 lg:w-[70%]'>
                        <div className='flex flex-col gap-y-4 md:flex-row md:flex-wrap md:items-center md:gap-x-0 xl:justify-between'>
                            <SmallCard
                                label={'Gender'}
                                value={patientFullData?.gender?.toLowerCase() ?? 'male'}
                            />
                            <SmallCard
                                label='Date of Birth'
                                value={
                                    patientFullData?.dateOfBirth
                                        ? format(patientFullData.dateOfBirth, 'yyyy-MM-dd')
                                        : 'N/A'
                                }
                            />
                            <SmallCard
                                label={'Phone Number'}
                                value={patientFullData?.phone ?? '0000000000'}
                            />
                        </div>
                        ...
                        <div className='flex flex-col gap-y-4 md:flex-row md:flex-wrap md:items-center md:gap-x-0 xl:justify-between'>
                            <SmallCard
                                ...
                                value={patientFullData?.nutritionalStatus ?? 'normal'}
                            />
                            <SmallCard
                                label='Blood Group'
                                value={patientFullData?.bloodGroup ?? 'A'}
                            />
                            <SmallCard
                                label='Address'
                                value={patientFullData?.address ?? 'hurghada'}
                            />
                        </div>
                        ...
                        <div className='flex flex-col gap-y-4 md:flex-row md:flex-wrap md:items-center md:gap-x-0 xl:justify-between'>
                            <SmallCard
                                ...
                                value={patientFullData?.emergencyContactName ?? 'N/A'}
                            />
                            <SmallCard
                                label='Emergency Contact'
                                value={patientFullData?.emergencyContactNumber ?? '0000000000'}
                            />
                            <SmallCard
                                label='Last Visit'
                                value={
                                    patientFullData?.lastVisit
                                        ? format(patientFullData?.lastVisit ?? 'N/A', 'yyyy-MM-dd')
                                        : 'No last visit'
                                }
                            />
                        </div>
