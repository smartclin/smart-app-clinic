import type { AppointmentStatus, Gender, Role } from '@prisma/client';
...
type AppointmentDetailsData = {
    ...
    time: string | null;
    note: string | null;
    status: AppointmentStatus; // Assuming AppointmentStatus is string-based
    patient: {
        id: string;
        email: string;
        createdAt: Date;
...
const AppointmentDetailsPage = async ({
    ...
    if (error) {
        ...
        return <div className='flex h-screen items-center justify-center text-red-500'>{errorMessage}</div>;
    }
