...
const MedicalRecordsPage = async ({ searchParams }: SearchParamsProps) => {
    // Await searchParams promise
    const params = await searchParams;

    const pageParam = typeof params?.['p'] === 'string' ? params['p'] : '1';
    const page = Number(pageParam) || 1;

    const searchQuery = (typeof params?.['q'] === 'string' ? params['q'] : '') as string;
    ...
    const {
        ...
        totalRecords = 0,
        currentPage = 1
    } = await api.medicalRecords.getMedicalRecords({ page, search: searchQuery });

    const data: ExtendedProps[] = rawData.map(item => ({
        ...item,
        patient: {
            ...item.patient,
            img: item.patient.image ?? ''
        },
