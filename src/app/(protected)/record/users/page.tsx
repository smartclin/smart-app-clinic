...
const columns = [
    ...
    } // Added for lastSignInAt
];

const UserPage = async () => {
    // Cast the result from auth.api.listUsers to ApiUser[]
    const { users, total } = (await authServer.listUsers({
        query: {
    ...
    const renderRow = (item: UserProps) => (
        ...
        >
            ...
            <td className='hidden capitalize xl:table-cell'>{item.emailVerified ? 'Verified' : 'Unverified'}</td>
