...
const BillingPage = async (props: SearchParamsProps) => {
    ...
    const data: PaymentRecord[] = rawData as unknown as PaymentRecord[];
    const session = await getSession();
    const isAdmin = checkRole(session, 'ADMIN');

    if (!data) {
        // Handle case where data might be null or undefined (though tRPC usually ensures an array)
        return (
            <div className='flex h-screen items-center justify-center text-gray-700'>No payment data available.</div>
        );
    }
    ...
    const renderRow = (item: PaymentRecord) => {
        ...
        return (
            ...
            >
                ...
                <td className='hidden xl:table-cell'>{(item?.totalAmount - (item?.discount ?? 0)).toFixed(2)}</td>
