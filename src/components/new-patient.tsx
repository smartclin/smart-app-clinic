...
import { GENDER, NUTRITIONAL_STATUS, RELATION } from '@/lib';
...
export const NewPatient = ({ data, type }: DataProps) => {
    ...
    const createPatientMutation = trpc.patient.createNewPatient.useMutation({
        onSuccess: res => {
            // You have access to the full response here.
            toast.success(res.message);
            form.reset();
            router.push('/patient'); // You can also access the router here.
        },
        // This is where you handle a failed mutation.
        onError: err => {
            // You have access to the full error object here.
    ...
    type AuthUser = {
        ...
        name?: string | null;
        email?: string | null;
        // Other properties from your authentication provider might also be here,
        // such as 'image' or 'providerAccountId'.
    };
    const getCreateDefaultValues = useCallback((user: AuthUser): PatientFormValues => {
        // Use an empty array as a fallback if user.name is null or undefined.
        // This prevents the TypeScript error when trying to use .slice() and .join() on a string.
    ...
    const getUpdateDefaultValues = useCallback(
        ...
        [getPersonalInfo, getFamilyInfo, getMedicalInfo]
    );
    ...
    return (
        <Card className='w-full max-w-6xl p-4'>
            <CardHeader>
                ...
                <CardDescription>
                    Please provide all the information below to help us understand better and provide good and quality
                    service to you.
                </CardDescription>
            </CardHeader>
