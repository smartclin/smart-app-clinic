...
function SidebarProvider({
    ...
    // This effect runs once on mount to read the initial persisted state
    React.useEffect(() => {
        const readPersisted = async () => {
            ...
            if ('cookieStore' in navigator) {
                try {
                    const cookie = await navigator.cookieStore.get(SIDEBAR_COOKIE_NAME);
                    if (cookie) {
                        _setOpen(cookie.value === 'true');
                        return;
                    }
                } catch (error) {
                    console.error('Failed to read sidebar cookie via Cookie Store API:', error);
                }
            }

            // Fallback: use localStorage (avoid document.cookie)
            try {
    ...
    const setOpen = React.useCallback(
        async (value: boolean | ((value: boolean) => boolean)) => {
            ...
            if (typeof window !== 'undefined' && 'cookieStore' in navigator) {
                try {
                    ...
                    console.error('Failed to set sidebar cookie via Cookie Store API:', error);
                }
            } else if (typeof window !== 'undefined') {
                // Fallback: use localStorage instead of document.cookie
                try {
                    window.localStorage.setItem(SIDEBAR_COOKIE_NAME, String(openState));
                } catch (error) {
                    console.error('Failed to persist sidebar state in localStorage:', error);
                }
            }
        },
        [setOpenProp, open]
    ...
    // Optional: keep multiple tabs in sync when using localStorage
    React.useEffect(() => {
        if (typeof window === 'undefined') return;
        const onStorage = (e: StorageEvent) => {
            if (e.key !== SIDEBAR_COOKIE_NAME || e.newValue == null) return;
            const nextOpen = e.newValue === 'true';
            // Only update if uncontrolled or we still want to propagate change outward
            if (setOpenProp) {
