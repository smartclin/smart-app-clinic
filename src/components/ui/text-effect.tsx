...
import {
    AnimatePresence,
    type MotionStyle, // <-- Add this import
    motion,
...
const AnimationComponent: React.FC<{
    ...
    const memoizedCharElements = React.useMemo(
        () =>
            segment.split('').map(char => (
                <motion.span
                    aria-hidden='true'
                    className='inline-block whitespace-pre'
                    key={`char-${segment}-${char}`}
                    variants={variants}
                >
                    {char}
                </motion.span>
    ...
    const content =
        per === 'line' ? (
            ...
            <motion.span className='inline-block whitespace-pre'>{memoizedCharElements}</motion.span>
        );
...
const splitText = (text: string, per: 'line' | 'word' | 'char') => {
    ...
    // Now the function uses the pre-defined regex constant.
...
const createVariantsWithTransition = (
    ...
    return {
        ...
        exit: {
            ...baseVariants['exit'],
            transition: {
                ...(baseVariants['exit'] && hasTransition(baseVariants['exit']) ? baseVariants['exit'].transition : {}),
                ...mainTransition,
                staggerDirection: -1
            }
        }
...
export function TextEffect({
    ...
    const segmentsWithKeys = React.useMemo(
        () => segments.map((segment, i) => ({ segment, key: `seg-${per}-${children}-${i}` })),
        [segments, per, children]
    );
        ...
        <AnimatePresence mode='popLayout'>
            {trigger && (
                ...
                >
                    ...
                    {segmentsWithKeys.map(({ segment, key }) => (
                        <AnimationComponent
                            key={key}
                            per={per}
                            segment={segment}
